...
home's `config.toml` (never `~/.codex`), and the next command reaches the host with no restart and
no task losing progress. Anything but `status: "ok"` — `okOverridden` included, or an error — is 409
