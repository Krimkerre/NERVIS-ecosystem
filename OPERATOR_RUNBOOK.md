...
| **A service running but not answering when the stack starts** | The menu bar app shows "Not answering: RAVIS" (or another service) after a start | Since 12 September the launcher does not launch a second copy over it: it waits the usual start-up time, then prints "RAVIS (process N) is running but not answering. Stop the stack, then start it again.", keeps the process in its PID file and exits 1. The menu bar app writes that sentence to `.run/menubar.log` rather than the menu | Quit NERVIS (or `tools/run.py stop`), which reaches the hung process through the PID file, then open it again |
