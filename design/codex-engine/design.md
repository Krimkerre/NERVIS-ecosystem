...
**Removed whenever the task rests, made again before each step (RAVIS 0.26.3).** Clarvis never ends a task — it settles every finished Codex task with `next: idle` and never calls `DELETE /api/v1/agent-sessions/{id}` — and RAVIS never expires an idle one, so 0.26.2's removal never ran for a normal task. The owner decided (15 September 2026) that the folder goes whenever a task has nothing running and comes back when Codex takes the next step, with tasks staying open for follow-ups. It is removed when the task rests: `idle` (settled with `next: idle` or `transfer`, or its thread resumed by a site reopen with nothing to run), every process confirmed gone. It is kept while a turn is starting, running, waiting on an answer or stopping, while `leftover`, and while a settle is owed (`stopped`, `completed_needs_review`, `paused_unanswered`, `paused_for_update`, `uncertain`), and so is any folder or parent another task of the project needs — one not resting, or with its action lock held. Before every `turn/start` (a carry-on, a follow-on a queued steer starts, a turn that waited for a reopen) and every `thread/resume` (after a restart, a site reopen, a resume create) RAVIS makes it again under the task's action lock, walking down from the project root without following a link, `0700` for the folder; if it can't, the step is refused — `503 CODEX_RUNTIME_UNAVAILABLE` for a turn or create, the waiting turn failed for a reopen, the steer not delivered for a follow-on — never run with a missing `TMPDIR`. The same safety rules as 0.26.2 apply to every removal. A resting task keeps its folder's name and the parents RAVIS made on record; RAVIS's start removes every resting task's folder. Calibration's and the re-test's own folders are not task folders and are unchanged.
