...
| **M21** | Notification centre — a durable store for things NERVIS wants to tell the user, with a screen, an unread count and dismissal | A finished task, a service transition and a question survive a reload and a restart; **every note says why it exists**, and one produced by a model also says which model and what it cost; dismissing is per-note and never bulk-silences a class; the voice announcement and the note are one event seen twice, not two events — muting speech must not lose the record |
...
| **M25** | Background thinking — NERVIS forms notes unattended, on its own pool and session, inside a spend ceiling | Runs on a pool distinct from chat's so the two do not contend, with **its own session id** so affinity never drags chat onto its model; a ceiling and an interval are configured and enforced, and the spend is attributable per pool in the usage ledger; the §9.6.1 background marker is **not** used, because "must be free" resolves to a local model and defeats the parallelism this exists for; **nothing it produces is an act** — every output is a note or a proposal, so §12 is untouched; turning it off leaves the rest of chat unchanged |
