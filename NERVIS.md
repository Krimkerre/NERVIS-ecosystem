...
API:            NERVIS API
Linux tray:     nervis-tray, opened from the applications menu
Windows:        inside WSL 2 only, as the Linux install, without the tray
```
...
telemetry may invoke the platform's native tools where necessary, on macOS or Linux.
...
| Same host, Safari | — | `NOT_TESTED` | No second browser was driven through the proxy. Safari was driven on 17 September 2026 on the *default* path — NERVIS's Code tab framing code-server at its own address: the panel rendered, the Bridge started on trust with no reload, and a chat turn went out through RAVIS and back (`clarvis/docs/code-server-matrix.md`, "Safari, through NERVIS's Code tab") — so it is evidence for that path, not this row |
...
| **M17** LIVE VERIFIED | Unified diagnostics — cross-service trace, health overlay, log correlation (by trace id, else by each line's own time within 2 s of the trace since NERVIS 0.34.20 — until then the fallback listed the newest lines, as no log line carried a time), benchmark/runtime context | A Clarvis → RAVIS → provider trace is visible; SIRVIS runtime evidence links where available; a broken link still produces a partial trace. **Witnessed whole on 16 September 2026:** the health overlay and log correlation were built and the SIRVIS evidence link seen live (`STATUS.md`, M17, 2 Sep); the last clause came from a Clarvis chat turn in code-server, which drew a Clarvis lane and a RAVIS lane under one trace id, with RAVIS's events naming the provider (Anthropic) and RAVIS's log line of the provider call correlated to the trace. `nervis.diagnostics@1` publishes itself available since NERVIS 0.34.4 |
...
**The owner's decision on the unbuilt parts, 18 September 2026.** Struck: most of §17's command
line and a doctor that checks its peers, and benchmark progress streamed rather than polled (it
waited on SIRVIS publishing progress, which is not wanted either). **Kept, to build:** §25.2's route
decision that can be linked to, and screens that update in place rather than redraw.
