...
| **M17** LIVE VERIFIED | Unified diagnostics — cross-service trace, health overlay, log correlation, benchmark/runtime context | A Clarvis → RAVIS → provider trace is visible; SIRVIS runtime evidence links where available; a broken link still produces a partial trace. **Witnessed whole on 16 September 2026:** the health overlay and log correlation were built and the SIRVIS evidence link seen live (`STATUS.md`, M17, 2 Sep); the last clause came from a Clarvis chat turn in code-server, which drew a Clarvis lane and a RAVIS lane under one trace id, with RAVIS's events naming the provider (Anthropic) and RAVIS's log line of the provider call correlated to the trace. `nervis.diagnostics@1` publishes itself available since NERVIS 0.34.4 |
