...
| **M19** IMPLEMENTED | macOS launcher — `NERVIS.app`, a menu bar app. **Rescoped 12 September 2026 by the owner:** one app that starts the whole stack from this repository, instead of a bundle per service, so a code change never needs repackaging. Built by `nervis/packaging/macos/build_app.sh` from `NERVISMenu.swift`; it holds the repository's path and runs `tools/run.py`, reading `run.py status --json` for everything it shows. No Dock icon: the NERVIS mark in the menu bar — its pupil solid while the stack answers, faint when part of it does not, blinking while NERVIS has unread notifications — and a menu with the unread count, the dashboard, each service, LM Studio and Ollama, CPU, GPU and memory, and a Quit that stops the stack. It opens the UI from that menu rather than on launch, by the owner's choice. **Seen live:** it started a stopped stack in 10 s; opened with the stack already up, it started nothing; its quit stopped all five services and left no process behind. **Not yet seen live:** the menu's own Open dashboard and Quit clicks, and a quit of the real stack with the build that fixed a hang on quit — that fix was verified against a stand-in launcher, where the build before it reproduced the hang | Launches the service, opens the UI, exits cleanly, respects independently running services, leaves no orphan process |
