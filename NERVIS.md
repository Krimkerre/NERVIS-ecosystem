...
first live Codex test ran one of the owner's personal skills unasked: between the allowed sites and
the version, every skill Codex can use as RAVIS 0.26.0 lists it (`GET /api/v1/codex/skills` through
the relay, outside `live()` like the sites), grouped *In NERVIS's skills folder*, *Your personal
skills* and *Built into Codex*, each with its description, whether it is on, and **Switch on** or
**Switch off**. One line on top names NERVIS's skills folder (`clarvis/skills` in the workspace) and
says a change counts from a Codex task's next start or reopen; RAVIS's `problem`, when the owner's
choices can't be applied, is shown as why no task can start. A switch takes one click — the card's
one exception to two, since one more click switches it back — never `confirm()`, shows *switching
on…* or *switching off…* until RAVIS answers, and goes through `POST /api/v1/ravis/codex/skills`,
which forwards only the page's `path` and `enabled` with NERVIS's RAVIS admin credential; RAVIS's
answer redraws the list, and a refusal (`SKILL_NOT_CHANGED`, `SKILL_NOT_FOUND`, Codex not running)
is said in words before the list is read again. What starts on and off is RAVIS's: NERVIS's folder
and Codex's built-in skills on, personal skills off. The launcher gives RAVIS the skills folder
from the same workspace path it gives NERVIS.
...
rather than deriving them (seventeen since NERVIS 0.30.0, ten of them Codex's: the sign-in's five
and the Codex card's five), so one
