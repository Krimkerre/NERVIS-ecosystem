...
| **M26** | **`ravis/background` — a pool for work with nobody watching.** Unattended callers (NERVIS's own thinking, M25 there) need a pool that runs *beside* chat rather than competing with it, and none of the existing thirteen does. `ravis/cheap` looks right and is the worst choice available: its `max_price_per_million` is `0.0`, so free-only, so local-only, so it loads a model onto the machine chat is already using. A low but **non-zero** ceiling instead, which lands on a small hosted model and genuinely parallelises | A background caller and a chat caller run concurrently without either loading a model for the other; the pool's spend is attributable in `/api/v1/usage` by pool, so "what did unattended work cost this week" is a query rather than an estimate; **§9.6.1's marker is not how this is used** — "must be free" resolves to local and defeats the pool's whole purpose, so the constraint is the ceiling and the ceiling is enforced |
