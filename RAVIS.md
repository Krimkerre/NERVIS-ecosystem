...
**As built, 16 September 2026 (RAVIS 0.29.0): tracked and published, not routed on.**
`/api/v1/health` carries `load` (`reliability/load.py`, `_load` in `api/management/routes.py`):

- **Active requests and local generations:** every upstream attempt is counted for as long as it
  runs, its stream included, by provider and model, on both execution paths (`_Call.running`), with
  the peak and the number of attempts since start.
- **Queue depth:** RAVIS keeps no queue of its own and says so; LM Studio and Ollama publish none,
  so theirs is `unknown`, and the local-generation count is the nearest honest figure.
- **Memory pressure:** the reading routing already samples.
- **Provider congestion:** each provider that has answered with a rate limit or an overload, how
  lately, and how often since start (`HealthRegistry.congestion`).
- **Provider quotas:** whatever a provider states on its answers — OpenAI's `x-ratelimit-*`,
  Anthropic's `anthropic-ratelimit-*`, a bare `x-ratelimit-*`, and `retry-after` — read by a
...
**Codex's skills — the owner's decisions of 14 September 2026 (0.26.0).** RAVIS starts Codex with
the owner's real `HOME`, so Codex found the owner's personal skills in `~/.agents/skills`, and the
first live task ran one nobody asked for. A NERVIS-only folder, `codex_skills_folder`
(`<NERVIS workspace>/clarvis/skills`, which the launcher names), now holds the ecosystem's skills:
RAVIS makes it with a README, names it with `skills/extraRoots/set`, and keeps its skills on.
Codex's built-in skills (`system` scope) start on; every other skill — the owner's personal ones, one
added later, `admin` skills, and any scope RAVIS doesn't know — starts off. Where a skill comes from
is decided by its real path, because an extra root's skills list as `user`, like the personal ones.
A project's own `repo` skills are Codex's to decide and aren't listed. The owner's switches live in
RAVIS's database (migration 11), not in Codex's `config.toml`, which is only where RAVIS applies them:
at every Codex start and again on `skills/changed`, with `skills/list` and then `skills/config/write`
by path until the list agrees. Until it does, or while the setting can't be used, no task starts (409
`CODEX_NOT_READY`; `runtime_down` with the reason), and neither does the re-test's or calibration's
turn. `GET /api/v1/codex/skills` (NERVIS's GET relay or admin) answers `SkillsView {folder, problem,
skills}`; `POST /api/v1/codex/skills {path, enabled}` (admin: NERVIS's Codex card) takes only a path
Codex lists just now (404 `SKILL_NOT_FOUND`), and puts the previous choice back when Codex doesn't
take the switch (409 `SKILL_NOT_CHANGED`). **No task root is, holds or lies inside the folder** (422
`WORKSPACE_ROOT_NOT_ALLOWED`, `denied_path`, naming the folder), so a Codex task never writes a skill
there; Clarvis's own engine, which RAVIS doesn't sandbox, still can while the owner has the workspace
...
**As built in R5 (0.25.0, 14 September 2026)**, in `src/ravis/agent/` and `api/management/codex.py`,
against the contract above. The sites routes read Codex's own list (the user layer of `config/read`)
on every call and write it with one `config/batchWrite` — an upsert for `POST`, a `replace` of the
rest for `DELETE` — and every site write, a site ask's and the defaults' included, waits for the one
before it, so a removal never drops a site allowed meanwhile. Site asks are grouped per turn
(`agent/sites.py`). The reopen is `agent/reopen.py` and the task's state machine: `thread/unsubscribe`
as the turn ends, `thread/loaded/list` every 2 s for at most 120 s, then `thread/resume` as after a
restart. A settle, a Stop or a switch skips only the resume, and the next turn resumes the thread; a
site allowed after the thread was resumed reopens it again, so a turn never starts in a thread loaded
before the task's last allowed site. `CreateSession.effort`, and a named model, are checked against
`model/list`; migration 10 stores the effort, sent on every `turn/start` (Codex 0.154.0 takes `effort`
...
**Migration 10 (R5, 0.25.0)** adds `agent_session.effort`: the reasoning effort a task was created
with, one of its model's efforts in Codex's `model/list`, or NULL for the model's own default. It is
...
| ~~**M17**~~ **CLOSED 18 September 2026** — owner's decision: NERVIS's twelve RAVIS screens are this dashboard; what they lacked — Profiles, a Models page, estimated savings, a live Clarvis compatibility page — is not wanted | Dashboard — Dashboard, Providers, Models, Profiles, Rules, Sessions, Routes, Usage, SIRVIS. **Mostly covered by NERVIS as of 12 September 2026, and not closed here:** NERVIS's RAVIS tab serves Dashboard, Providers, Sessions, Routes, Spending for usage, Policies for rules and Evidence for what SIRVIS measured, plus Pools, Credentials, Logs, Diagnostics and Settings. It has no Profiles or Models screen (§15.3). Whether that closes M17 is the owner's call | — |
