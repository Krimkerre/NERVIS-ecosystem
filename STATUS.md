...
| **NERVIS M5a** | SIRVIS's read surfaces with provenance intact and no benchmark logic of NERVIS's own. **M5b was unblocked** when SIRVIS M14's job queue shipped — submit and cancel run through it as §12 operations; a run is polled rather than streamed, which is the degradation's stated reason | `nervis/tests/test_m5_sirvis.py`, `nervis.sirvis_views@1` **degraded** |
