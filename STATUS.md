...
.venv/bin/pytest                      # part of 1449 tests, no network, no live service
...
cd nervis   && ../ravis/.venv/bin/python -m pytest -q   # 285 tests
```

**`ecosystem-protocol` must be installed first.** It is a local path dependency
and pip will not find it on PyPI, because it does not live there.

Expected: all clean, 1449 passing across the four, conformance `PASS`. CI runs the same four on
